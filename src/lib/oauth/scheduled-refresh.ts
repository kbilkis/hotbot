...
import { gitProviders, messagingProviders } from "../database/schema";
...
export async function refreshExpiringTokens(): Promise<{
  ...
  try {
    ...
    for (const provider of expiringGitProviders) {
      try {
        const success = await refreshGitProviderToken(
          provider.id,
          provider.provider
        );
        if (success) {
    ...
    for (const provider of expiringMessagingProviders) {
      try {
        const success = await refreshMessagingProviderToken(
          provider.id,
          provider.provider
        );
        if (success) {
    ...
    const errorMsg = `Critical error in scheduled token refresh: ${error}`;
    console.error(errorMsg);
    errors.push(errorMsg);
...
async function refreshGitProviderToken(
  ...
  try {
    ...
    if (!providerData || !providerData.refreshToken) {
      console.warn(`No refresh token available for git provider ${providerId}`);
      return false;
    }
    ...
    switch (provider) {
      ...
      case "github":
        ...
        console.log("GitHub tokens typically don't require refresh");
        return true;
      case "bitbucket":
    ...
    console.log(`Successfully refreshed token for git provider ${providerId}`);
    return true;
  } catch (error) {
    console.error(`Failed to refresh git provider token ${providerId}:`, error);
    return false;
  }
}
...
async function refreshMessagingProviderToken(
  ...
  try {
    ...
    if (!providerData || !providerData.refreshToken) {
      console.warn(
        `No refresh token available for messaging provider ${providerId}`
      );
      return false;
    }
    ...
    switch (provider) {
      ...
      case "slack":
        ...
        console.log("Slack tokens don't require refresh");
        return true;
      case "teams":
    ...
    console.log(
      `Successfully refreshed token for messaging provider ${providerId}`
    );
    return true;
  } catch (error) {
    console.error(
      `Failed to refresh messaging provider token ${providerId}:`,
      error
    );
    return false;
  }
